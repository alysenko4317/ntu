...
public class CarDAO_JdbcImpl implements CarDAO
{
    ...
    private RowMapper<Car> carRowMapper = (ResultSet rs, int i) ->
    {
        ...
        if (! rs.wasNull())
        {
            ...
            // передаємо порожній список автомобілів, якими володіє користувач (new ArrayList<Car>())
            // це означає, що у користувача є автомобілі у власності (в принципі),
            // але ми тут не хочемо виконувати додаткову роботу - витягувати з БД всі автомобілі,
            // якими користувач потенційно може володіти, але хоча б одним (тим, що ми створили)
            // він точно володіє, тому передаємо new ArrayList<Car>(), а не null;
            // до чого тут null - читай коментар до accountRowMapper в AccountDAO_JdbcImpl
        }

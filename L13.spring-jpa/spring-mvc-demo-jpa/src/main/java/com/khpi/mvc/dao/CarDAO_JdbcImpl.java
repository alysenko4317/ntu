    ...
    private RowMapper<Car> carRowMapper = (ResultSet rs, int i) ->
    {
        ...
        if (! rs.wasNull())
        {
            account = new Account(
                ...
                rs.getString("email"),
                rs.getString("password"), new ArrayList<Car>(), 0);

            // передаємо порожній список машин, якими володіє користувач (new ArrayList<Car>())
            // це означає, що користувач має машини у власності (в принципі),
            // але тут ми не хочемо виконувати додаткову роботу - витягувати з БД усі машини,
            // якими користувач потенційно може володіти, але хоча б однією (тією, що ми створили)
            // він точно володіє, тому передаємо new ArrayList<Car>(), а не null;
            // причому щодо null - дивись коментар до accountRowMapper в AccountDAO_JdbcImpl
        }
